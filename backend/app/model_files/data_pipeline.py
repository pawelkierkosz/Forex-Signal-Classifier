...
N_PIVOTOW_SR = 8
SAVE_PIVOTS_CSV = False

try:
    from .config import MT5_CSV
except Exception:
    try:
        from config import MT5_CSV
    except Exception:
...
if isinstance(MT5_CSV, Path):
    PIVOTS_CSV = MT5_CSV.with_name("pivots_ZZ.csv")
else:
    PIVOTS_CSV = Path("pivots_ZZ.csv")
...
def full_ZZ(df_swieczki, ileZZ, minzmnpkt, backstep):
    ...
    df_pivots = pd.DataFrame(pivots, columns=["bar", "price", "type"])

    if SAVE_PIVOTS_CSV and PIVOTS_CSV is not None:
        try:
            PIVOTS_CSV.parent.mkdir(parents=True, exist_ok=True)
            df_pivots.to_csv(PIVOTS_CSV, index=False)
        except Exception as e:
            print(f"[full_ZZ] Uwaga: nie udało się zapisać pivotów do {PIVOTS_CSV}: {e}")

    return df_pivots


def update_zigzag(bar, last_row, last_count, H, L, ileZZ, minzp, backstep):
    idx = bar - 1
...
def label_next_pivot(sw, df_zz_all, O:pd.Series):
    try:
...
def compute_avg_pivot_diffs(df_zz_all, sw_val, n_pivotow):
...
# -- Budowa cech --
def prepare_features(df_swieczki, df_zz_all, od, do, dl_zz, liczba_sw, ile_zz, min_zp, backstep, n_pivotow=None):
    H = df_swieczki["H"].values
    L = df_swieczki["L"].values
    C = df_swieczki["C"]
    ...
    for sw in range(od, do+1):
        ...
        if n_pivotow is not None and n_pivotow > 0:
            sr_wys, sr_szer = compute_avg_pivot_diffs(df_zz_all, sw, n_pivotow)
        ...
        lab, piv_bar = label_next_pivot(sw, df_zz_all, df_swieczki["O"])
...
# -- Wskaźniki --
def calculate_technical_indicators(df_swieczki, od, do, m):
...
def combine_features(df_ceny, df_wskazniki):
...
# -- Normalizacja --
def normalize_features(W:pd.DataFrame, O:pd.Series, dl_zz:int, liczba_sw:int, stats=None):
...
# -- Cechy dla ostaniej zamkniętej świecy --
def features_last_bar(df_swieczki, MIN_ZP, ILE_ZZ, BACKSTEP, DL_ZZ, LICZBA_SW, norm_stats):
    ...
    df_in, df_out, first_bar = prepare_features(
        df_swieczki,
        df_ZZ,
        od,
        do,
    ...
    df_ind = calculate_technical_indicators(df_swieczki, od, do, len(df_in))
    df_all = combine_features(df_in, df_ind)

    sw_index = np.arange(first_bar, do + 1)
    df_all.index = sw_index

    O_next_series = df_swieczki["O"].shift(-1)
    ...
    for bar_idx in sorted(ready_idx, reverse=True):
        ...
        X_norm, _ = normalize_features(X_raw, O_next, DL_ZZ, LICZBA_SW, stats=norm_stats)
        if not X_norm.isna().any().any():
            sr_wys, sr_szer = compute_avg_pivot_diffs(df_ZZ, bar_idx, N_PIVOTOW_SR)
